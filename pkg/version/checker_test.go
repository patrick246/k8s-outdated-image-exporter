...
func TestChecker_GetDifference(t *testing.T) {
	...
	for _, testcase := range testCases {
		t.Run(fmt.Sprintf("current=%s;available=%s", testcase.Current, testcase.Available), func(t *testing.T) {
			major, minor, patch, err := versionChecker.GetDifference(testcase.Current, testcase.Available)
			require.NoError(t, err)
